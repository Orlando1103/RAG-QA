...
from src.rag_qa.data.datasets import prepare_questions, write_prepare_report


def main() -> None:
    """
    执行命令时，真正跑起来的 “总入口”。负责：接收命令参数 → 读取配置 → 准备问题数据 → 输出结果。
    """
    ##TODO:argparse知识学习
    ...
    config = load_config(args.config) #返回AppConfig类
    questions = prepare_questions(config, force_refresh=args.force_refresh) #dataset函数准备问题
